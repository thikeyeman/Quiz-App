...
function appendQuestion() {
  ...
  divBody += "</ul>"
  divBody += "<button class='btn btn-primary nxtBtn' onclick='nxtQuestion()'>次へ</button>"
...
function uploadResults(username, correctQuestions, totalQuestions, timeTaken) {
  // Get a reference to the "results" node in the database
  const resultsRef = firebase.database().ref("results");

  // Push the results to the database (as a new child node)
  resultsRef.push({
    username: username,
